...
const RequsetPage = () => {
  ...
  const onServiceChange = (value) => {
    ...
    request("get", "requests/getId/" + value, {}).then((res) => {
      if (res.status === 200) {
        setLoading(false);
        var data = res.data;
        setid(data.data);
    ...
    filtered.map((requests) => {
      if (requests.service_id.includes(value)) {
        var arrTmpPro = requests.problem.map((items) => ({
          label: items,
          value: items,
        }));
        setProblem(arrTmpPro);
        ...
        request("get", "Em/getEmById/" + requests.isOusers.em_ID, {}).then(
          (res) => {
            setLoading(false);
            if (res.status === 200) {
              var data = res.data;
        ...
        request(
          "get",
          "Em/getAllEmAndShEmail/" + requests.isOusers.em_ID,
          {}
        ).then((res) => {
          setLoading(false);
          if (res.status === 200) {
  ...
  const getListService = () => {
    ...
    request("get", "admin/service", {}).then((res) => {
      ...
      if (res.status === 200) {
        ...
        var arrTmpS = data.data.ServiceDetail.map((item) => ({
  ...
  const getListCategorization = () => {
    ...
    request("get", "admin/categorization", {}).then((res) => {
      ...
      if (res.status === 200) {
        var data = res.data;
        var arrTmpC = data.data.map((item) => ({
  ...
  const getListPriority = () => {
    ...
    request("get", "admin/priority", {}).then((res) => {
      ...
      if (res.status === 200) {
        var data = res.data;
        var arrTmpP = data.data.map((item) => ({
  ...
  const onFinish = async (item) => {
    ...
    var body = {
      saveRequestDto: {
        ...
        status: "1",
        reqProblem: item.problem.toString(),
        priorityID: item.priorityID,
        cateo_ID: item.cateo_ID,
        pro_ID: "1",
        service_ID: item.service_ID,
        userContact: item.userContact,
        createDate: dayjs(date).format("YYYY-MM-DDTHH:mm:ss"),
        incidentDate: dayjs(incidentDate).format("YYYY-MM-DDTHH:mm:ss"),
      },
      requestDetail: {
        reqForMySelf: value1,
          ...
          >
                  ...
                  <Col span={8} className="classA">
                    ...
                    >
                      <Input
                        className="classB"
                        bordered={false}
                        value={dayjs(createDate).format("MMYYYY") + "~" + id}
                      />
                    </Form.Item>
                  </Col>
                  <Col span={8} className="classA">
                  ...
                  <Col span={8} className="classA">
                    ...
                    <Form.Item
                      ...
                      rules={[
                        {
                          required: true,
                          message: "Please select incident date!",
                        },
                      ]}
                    >
            ...
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: "Please select type Of requset",
                },
              ]}
            >
              <Select
            ...
            <Row gutter={16}>
              ...
              <Col span={6}>
                <Form.Item labelCol={{ style: { width: 200 } }} label="Section">
                  <Input
              ...
              <Col span={8}>
                <Form.Item name="reqForMySelf">
                  ...
                  >
                    <Space size={30}>
                      <Radio value="Y" onChange={onReset}>
                        {" "}
                        Requset for myself{" "}
                      </Radio>
                      <Radio
                        value="N"
                        onChange={(e) => setComponentDisabled(false)}
                      >
                        {" "}
                        Requset for other{" "}
                      </Radio>
              ...
              <Col span={12}>
                <Row gutter={16}>
                  <Col span={8}>
                    <Form.Item name="reqForMyEmId">
                      <Input
                        disabled={componentDisabled}
                        onChange={onChangeReqFor}
                        style={{ width: "100%" }}
                        placeholder="ID"
                      />
                    </Form.Item>
              ...
              <Col span={8}>
                <Form.Item
                  ...
                  name="emailToEm"
                >
                  <Input style={{ width: "100%" }} prefix={<MailTwoTone />} />
                </Form.Item>
              </Col>
